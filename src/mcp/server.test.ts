...
vi.mock('../external/spotify/index.ts');
...
describe('mcpServer', () => {
  ...
  describe('search tool', () => {
    it('should return track results', async () => {
      ...
      vi.mocked(spotifyApiSdk.searchTracks).mockResolvedValueOnce(ok(mockTracks));
      vi.mocked(spotifyApiSdk.createSpotifyClient).mockReturnValue({} as any);
      ...
      expect(spotifyApiSdk.createSpotifyClient).toHaveBeenCalledWith('mock-access-token');
      expect(spotifyApiSdk.searchTracks).toHaveBeenCalledWith({}, 'test song', 10);
    });

    it('should handle search errors', async () => {
      vi.mocked(spotifyApiSdk.searchTracks).mockResolvedValueOnce(
        err({
          type: 'NetworkError',
    ...
    it('should handle custom limit parameter', async () => {
      ...
      expect(spotifyApiSdk.searchTracks).toHaveBeenCalledWith(undefined, 'test', 5);
    });
  });
  ...
  describe('player_state tool', () => {
    it('should return current state', async () => {
      ...
      vi.mocked(spotifyApiSdk.getCurrentPlayback).mockResolvedValueOnce(ok(mockPlayerState));
    ...
    it('should handle no active playback', async () => {
      vi.mocked(spotifyApiSdk.getCurrentPlayback).mockResolvedValueOnce(ok(null));
    ...
    it('should handle playback errors', async () => {
      vi.mocked(spotifyApiSdk.getCurrentPlayback).mockResolvedValueOnce(
        err({
          type: 'AuthError',
  ...
  describe('player_control tool', () => {
    it('should execute play command', async () => {
      vi.mocked(spotifyApiSdk.controlPlayback).mockResolvedValueOnce(ok(undefined));
      ...
      expect(spotifyApiSdk.controlPlayback).toHaveBeenCalledWith(undefined, 'play', undefined);
    });
    ...
    it('should execute pause command', async () => {
      ...
      expect(result.isOk()).toBe(true);
      expect(spotifyApiSdk.controlPlayback).toHaveBeenCalledWith(undefined, 'pause', undefined);
    });
    ...
    it('should execute next command', async () => {
      ...
      expect(result.isOk()).toBe(true);
      expect(spotifyApiSdk.controlPlayback).toHaveBeenCalledWith(undefined, 'next', undefined);
    });
    ...
    it('should execute previous command', async () => {
      ...
      expect(result.isOk()).toBe(true);
      expect(spotifyApiSdk.controlPlayback).toHaveBeenCalledWith(undefined, 'previous', undefined);
    });
    ...
    it('should handle playback control errors', async () => {
      vi.mocked(spotifyApiSdk.controlPlayback).mockResolvedValueOnce(
        err({
          type: 'SpotifyError',
